...
public class EntityInsertHandler extends AbstractEntityHandler {
    public EntityInsertHandler(DataSource dataSource, EntityTransform entityBinding, CallBack callBack) {
        super(dataSource, entityBinding, callBack);
    }

    @Override
    public <T> T process(T entity) {
        final QueryBuilder query = EntityQueryBuilderFactory.getQueryBuilder(ProcessQueryType.INSERT, entity);
        try {
            super.execute(query.sql(), query.params());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
