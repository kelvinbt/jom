...
public class EntitySelectHandler extends AbstractEntityHandler {
    public EntitySelectHandler(DataSource dataSource, EntityTransform entityBinding, CallBack callBack) {
        super(dataSource, entityBinding, callBack);
    }

    @Override
    public <T> T process(T entity) {
        final QueryBuilder builder = EntityQueryBuilderFactory.getQueryBuilder(ProcessQueryType.SELECT, entity);
        try {
            return query(builder.sql(), builder.params());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
