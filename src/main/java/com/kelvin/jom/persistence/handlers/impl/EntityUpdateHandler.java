...
public class EntityUpdateHandler extends AbstractEntityHandler {
    public EntityUpdateHandler(DataSource dataSource, EntityTransform entityBinding, CallBack callBack) {
    ...
    public <T> T process(T entity) {
        final QueryBuilder builder = EntityQueryBuilderFactory
                .getQueryBuilder(ProcessQueryType.UPDATE, entity);
        try {
            return execute(builder.sql(), builder.params());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
